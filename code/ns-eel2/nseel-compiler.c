...
static float NSEEL_CGEN_CALL _eel_flacDecodeFile(void *opaque, INT_PTR num_param, float **parms)
{
	...
	uint64_t frameCount;
	        float *signal = drflac_open_file_and_read_pcm_frames_f32(filename, &channels, &fs, (drflac_uint64 *)&frameCount, 0);	float targetFs = *parms[2];
...
static float NSEEL_CGEN_CALL _eel_flacDecodeMemory(void *opaque, INT_PTR num_param, float **parms)
{
	...
	uint32_t channels, fs;
	uint64_t frameCount;
	        float *signal = drflac_open_memory_and_read_pcm_frames_f32(memoryBlk, actualSize, &channels, &fs, (drflac_uint64 *)&frameCount, 0);	float targetFs = *parms[2];
	if (targetFs > FLT_EPSILON)
	{
		double ratio = targetFs / (double)fs;
		if (ratio != 1.0)
